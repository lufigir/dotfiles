...
- `powershell/Microsoft.PowerShell_profile.ps1` — PowerShell profile (Oh My Posh init + Linux-style aliases; `touch`/`which`/`grep` are defined on Windows only, since on macOS/Linux the real binaries are on PATH and beat these stand-ins)
